...
- Cursor Agent (`xTacobaco/cursor-agent.nvim`)
  - Normal `<leader>ac`: `:CursorAgent` 토글 (열기/닫기)
  - Visual `<leader>ac`: `:CursorAgentSelection` (선택한 텍스트만 전송)
  - Normal `<leader>aC`: `:CursorAgentBuffer` (현재 버퍼 전체 텍스트 전송)
  - `cursor-agent` CLI가 없으면 경고 메시지 표시
- 창 닫기
  - `<leader>ac`를 다시 눌러 토글로 닫기
  - 또는 창에서 `q` (터미널 입력 모드면 `Ctrl-\` `Ctrl-n` 후 `q`)
- 용어 정리
